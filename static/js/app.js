...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // (This section is unchanged, condensed for brevity)
    ...
    // --- FEATURE: COLLECTION WITH RARITY, STATS, AND TEAM MANAGEMENT ---
    function updateCollectionDisplay() {
        ...
        gameState.collection.forEach(char => {
            ...
            card.innerHTML = `
                <div class="card-rarity rarity-${charDef.rarity.toLowerCase()}">[${charDef.rarity}]</div>
                ...
                <button class="set-team-button" data-char-id="${char.id}" data-action="${isInTeam ? 'remove' : 'add'}">
    ...
    function updateCampaignDisplay() { /* ... unchanged ... */
    ...
    async function initializeGame() {
        ...
        if (loggedIn) {
            ...
            loginScreen.classList.remove('active');
            gameScreen.classList.remove('active'); // Typo here, should be loginScreen active
    ...
    registerButton.addEventListener('click', async () => { /* ... unchanged ... */
    ...
    summonButton.addEventListener('click', async () => { /* ... unchanged ... */
        const response = await fetch('/api/summon', { method: 'POST' });
        const result = await response.json();
        if (result.success) {
            const character = result.character;
            summonResultContainer.innerHTML = `<div class="team-slot"><img src="/static/images/characters/${character.image_file}" alt="${character.name}"><h4>[${character.rarity}] ${character.name}</h4><p>ATK: ${character.base_atk} | HP: ${character.base_hp}</p></div>`;
    ...
    // --- FEATURE: NEW EVENT HANDLER FOR TEAM MANAGEMENT ---
    document.body.addEventListener('click', async (e) => {
        // Fight Button Logic (unchanged)
        ...
        // Add/Remove from team logic
        if (e.target && e.target.classList.contains('set-team-button')) {
            const charId = parseInt(e.target.dataset.charId, 10);
            const action = e.target.dataset.action; // 'add' or 'remove'
            const response = await fetch('/api/manage_team', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ char_id: charId, action: action })
            });
            const result = await response.json();
            if(result.success) {
                await fetchPlayerDataAndUpdate();
            } else {
                alert(`Failed to update team: ${result.message}`);
            }
        }
    });

    // Navigation logic (with summon screen fix)
